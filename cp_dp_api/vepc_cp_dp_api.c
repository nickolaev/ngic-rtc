...
build_dp_msg(enum dp_msg_type mtype, struct dp_id dp_id,
					void *param, struct dp_ipc_msgbuf *msg_payload)
{
 ...
 */
static int
send_dp_msg(struct dp_id dp_id, struct dp_ipc_msgbuf *msg_payload)
{
	RTE_SET_USED(dp_id);
	if (active_comm_msg->send((void *)msg_payload, sizeof(struct dp_ipc_msgbuf)) < 0) {
		perror("msgsnd");
		return -1;
	}
	return 0;
...
{
#ifdef CP_BUILD
	struct dp_ipc_msgbuf msg_payload;
