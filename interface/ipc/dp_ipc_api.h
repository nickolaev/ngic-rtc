...
struct dp_ipc_msgbuf sbuf;
struct dp_ipc_msgbuf rbuf;
/* IPC msg node */
struct ipc_node {
	int msg_id;	/*msg type*/
	int (*msg_cb)(struct dp_ipc_msgbuf *msg_payload);	/*callback function*/
};
...
iface_ipc_register_msg_cb(int msg_id,
		int (*msg_cb)(struct dp_ipc_msgbuf *msg_payload));
